...
import { broadcast } from './messageHandler'

/**
 * @typedef {Object} Client
 * @property {String} id Client ID
 * @property {String} name Client's name
 * @property {Object} wsClient WebSocket client object
 */
/**
 * @typedef {Object} Room
 * @property {String} id Room ID
 * @property {String} name Room's name
 * @property {Number} size Room's available places
 * @property {Client[]} clients Room's clients
 */
/**
...
const checkRoomExist = roomId => {
  if (!rooms[roomId]) throw new Error('Could not find the room')
}
...
export const newRoom = (roomSize, roomName = 'No name') => {
  ...
  const roomId = uuid()
  rooms[roomId] = { name: roomName, size: roomSize, clients: [] }
  return roomId
}
 ...
 * @param {String} wsClient WebSocket client to add to the room
...
export const exitRoom = (roomId, clientId) => {
  ...
  // Delete the room if it will be empty after quitting it
  if (rooms[roomId].clients.length - 1 <= 0) return delete rooms[roomId]

  // Exit the room if not deleted yet
